...
export default function Transactions() {
    ...
    return <AppLayout>
        ...
        <TransactionModal open={transactionModalOpen || Boolean(transactions.editingTransaction)} onClose={() => { setTransactionModalOpen(false); transactions.setEditingTransaction(null); }} form={transactions.form} accounts={transactions.accounts} creditCards={transactions.creditCards} categories={transactions.categories} merchants={transactions.merchants} onSubmit={async (event) => { const requestError = await transactions.submitTransaction(event); if (!requestError) setTransactionModalOpen(false); }} onCreateMerchant={() => transactions.setMerchantOpen(true)} submitting={transactions.submitting} apiErrors={transactions.apiErrors} editing={transactions.editingTransaction} />
