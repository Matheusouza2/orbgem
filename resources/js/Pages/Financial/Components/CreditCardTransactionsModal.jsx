...
export default function CreditCardTransactionsModal({ card, transactions, meta, loading, error, filters, onClose, onFilter, onPage }) {
    return <Modal show={Boolean(card)} onClose={onClose} dismissible={!loading} size="4xl" className="transactions-history-modal wallet-modal">
        <ModalHeader className="wallet-modal__header">
            <CreditCardIcon institution={card?.institution} />
            <span className="wallet-modal__heading">
                <span className="wallet-modal__eyebrow">Movimentos do cartão</span>
                <span className="wallet-modal__title">{card?.name}</span>
            </span>
        </ModalHeader>
