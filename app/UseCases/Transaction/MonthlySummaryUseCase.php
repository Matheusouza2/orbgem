...
class MonthlySummaryUseCase
{
    ...
    public function execute(MonthlySummaryDTO $summaryDTO, User $user): array
    {
        ...
        $balance = $this->accountService->listForWallet($summaryDTO->walletId)->sum(fn ($account): int => $this->balanceCalculator->calculate($account->initial_balance, $this->transactionService->postedAmountsForAccount($account->id)));
