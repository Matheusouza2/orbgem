...
class CreditCardResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        ...
        return ['id' => $this->id, 'wallet_id' => $this->wallet_id, 'name' => $this->name, 'institution' => $this->institution, 'limit' => $limit, 'current_invoice_amount' => $invoiceAmount, 'current_invoice_reference_month' => $currentInvoice?->reference_month, 'limit_usage_percentage' => $limit > 0 ? round(($invoiceAmount / $limit) * 100, 2) : 0, 'previous_invoice_amount' => $previousInvoiceAmount, 'previous_invoice_reference_month' => $previousInvoice?->reference_month, 'previous_invoice_usage_percentage' => $limit > 0 ? round(($previousInvoiceAmount / $limit) * 100, 2) : 0, 'closing_day' => $this->closing_day, 'due_day' => $this->due_day, 'active' => $this->active, 'created_at' => $this->created_at];
    }
